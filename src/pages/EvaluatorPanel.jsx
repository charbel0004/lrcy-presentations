  ...
  const loadAll = async () => {
    ...
    try {
      const [p, c] = await Promise.all([
        api("/api/evaluator/presenters"),
        api("/api/evaluator/evaluation-criteria"),
      ]);
    ...
    <div className="min-h-screen bg-neutral-950 text-white">
          ...
          <div className="rounded-2xl bg-neutral-900/60 border border-neutral-800 p-5">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-neutral-400 mt-1">
                  Evaluations require scoring all active criteria (1–10 each).
                </div>
              </div>
              <div className="text-xs text-neutral-300">
                {criteria.length ? `${criteria.length} active criteria` : "No active criteria"}
              </div>
            </div>
            ...
            {criteria.length ? (
              ...
              <div className="mt-3 text-sm text-neutral-400">
                Ask the admin to add/activate criteria in the Admin Panel.
              </div>
            )}
          </div>
      ...
      {activePresenter ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
          ...
          {/* CHANGED: modal becomes flex-column with max height and scrollable body */}
          <div className="relative w-full max-w-2xl rounded-2xl bg-neutral-950 border border-neutral-800 shadow-xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-5 border-b border-neutral-800 flex items-start justify-between gap-4 shrink-0">
              <div className="min-w-0">
                ...
                <div className="text-xs text-neutral-500 mt-1 truncate">
                  Topic: {activePresenter.topic?.title || "-"}
                </div>
              </div>
              <button
            ...
            <div className="p-5 space-y-4 flex-1 overflow-y-auto">
              {!criteria.length ? (
                <div className="text-sm text-neutral-400">
                  No active criteria. Ask admin to activate criteria first.
                </div>
              ) : (
                    ...
                    <div key={c._id} className="rounded-xl bg-neutral-900/40 border border-neutral-800 p-4">
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                        <div className="min-w-0">
                          ...
                          {c.description ? (
                            <div className="text-xs text-neutral-500 mt-1 break-words">{c.description}</div>
                          ) : null}
                        </div>
            ...
            {/* Footer (sticky so Submit is always reachable on mobile) */}
            <div className="sticky bottom-0 p-5 border-t border-neutral-800 bg-neutral-950/95 backdrop-blur shrink-0">
                ...
                <button
                  ...
                  className={[
                    ...
                    !criteria.length || !allScored
                      ? "bg-neutral-900/40 border border-neutral-800 text-neutral-500 cursor-not-allowed"
              ...
              {!allScored && criteria.length ? (
                <div className="mt-2 text-xs text-neutral-500">
                  You must score every criterion before submitting.
                </div>
              ) : null}
            </div>
          </div>
